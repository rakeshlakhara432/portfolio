import React from 'react';

const Projects = () => {
  const projectList = [
    {
      title: 'E-Commerce Dashboard',
      desc: 'A modern, responsive admin dashboard for managing products, users, and orders with full CRUD functionality.',
      tech: ['React', 'Chart.js', 'Netlify'],
      img: 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80',
      github: '#'
    },
    {
      title: 'Real Estate Platform',
      desc: 'A beautifully designed real estate portal to search, filter, and view property listings with high-res image galleries.',
      tech: ['React', 'CSS Modules', 'Firebase'],
      img: 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80',
      github: '#'
    },
    {
      title: 'Crypto Tracker App',
      desc: 'Live cryptocurrency dashboard pulling data from CoinGecko API featuring sparkline charts and live market cap tracking.',
      tech: ['React', 'Axios', 'Tailwind'],
      img: 'https://images.unsplash.com/photo-1621504450181-5d356f61d307?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80',
      github: '#'
    }
  ];

  return (
    <section id="projects" className="section container">
      <h2 className="section-title">Some Things I've Built</h2>
      <div className="projects-grid">
        {projectList.map((project, idx) => (
          <div key={idx} className="project-card glass">
            <div className="project-img-wrapper">
              <img src={project.img} alt={project.title} className="project-img" />
              <div className="project-img-overlay">
                <a href={project.github} className="project-link-btn" aria-label="GitHub Link">
                  <svg width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                    <path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.166 6.839 9.489.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.699-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.462-1.11-1.462-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.564 9.564 0 0 1 12 6.844c.85.004 1.705.114 2.504.336 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.161 22 16.418 22 12c0-5.523-4.477-10-10-10z"></path>
                  </svg>
                </a>
              </div>
            </div>
            <div className="project-content">
              <h3 className="project-title">{project.title}</h3>
              <p className="project-desc">{project.desc}</p>
              <div className="project-tech">
                {project.tech.map((t, index) => (
                  <span key={index} className="tech-tag">{t}</span>
                ))}
              </div>
            </div>
          </div>
        ))}
      </div>
    </section>
  );
};

export default Projects;
